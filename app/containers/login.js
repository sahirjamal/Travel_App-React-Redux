...
class Login extends React.Component {
  ...
  authenticate() {
    firebase.auth().signInWithEmailAndPassword(this.state.email, this.state.password).catch((error) => {console.log(error)});
  }
    ...
    return (
      <Container className='loginPage'>
        <Form>
          ...
          <Button onClick={() => {this.authenticate()}} type='submit'>Login</Button>
        </Form>
      </Container>
    )
  }
